...
module Game_state = struct
  ...
  (* Check if a cell position is a corner cell (not allowed in Pixel game) *)
  let is_corner_cell { rows; columns; _ } ({ row; column } : Cell_position.t) =
    (row = 0 && column = 0) ||
  ...
  let get_all_moves t : Move.t list =
    ...
    |> List.filter ~f:(fun move -> not (is_corner_cell t move))
  ;;
  ...
  let make_move t (cell_position : Move.t) : (t, Move_error.t) Result.t =
    ...
    | _ when is_corner_cell t cell_position -> Error Corner_cell_not_allowed
      ...
      (match Map.find t.board cell_position with
       ...
       | None ->
         ...
         let decision : Decision.t =
           ...
           | None ->
             if Map.length board >= (t.columns * t.rows) - 4 (* Subtract 4 for corner cells *)
             then Stalemate
